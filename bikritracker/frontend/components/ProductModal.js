...
export default function ProductModal({ initial, onClose, onSave }) {
  ...
  const addHistory = () =>
    setProduct({
      ...product,
      history: [...(product.history || []), { Orderdate: "", Sales: "" }],
    });
  ...
  const save = () => {
    ...
    const hist = (product.history || []).map((h) => ({
      ...
      <div className="modal">
        ...
        <div className="stacked-row">
          ...
          <div>
            ...
            >
              <label className="kv">
                Sales history (add at least 30 rows)
              </label>
              <button className="btn secondary" onClick={addHistory}>
                + Add row
              </button>
            </div>
            ...
            {(product.history || []).map((h, i) => (
              <div key={i} className="history-row" style={{ marginTop: 8 }}>
                <input
                  ...
                  className="input"
                  value={h.Orderdate}
                  onChange={(e) => updateHistory(i, "Orderdate", e.target.value)}
                />
                <input
                  type="number"
                  placeholder="Sales"
                  className="input"
                  value={h.Sales}
                  onChange={(e) => updateHistory(i, "Sales", e.target.value)}
                />
                <button className="btn danger" onClick={() => removeHistory(i)}>
                  Delete
                </button>
              </div>
            ))}
            <div className="small" style={{ marginTop: 6 }}>
              Tip: Enter at least 30 rows — one per day/week/month depending on
              your data.
            </div>
          </div>
